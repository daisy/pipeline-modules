...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/*">
        <smil>
            <xsl:variable name="timeInThisSmil" select="sum(//body/seq/pf:clockValToSeconds(@dur))"/>
            <head>
                <meta name="ncc:timeInThisSmil"
                    content="{pf:secondsToFullClockVal($timeInThisSmil)}"
            ...
            <body>
                <seq dur="{pf:secondsToTimecount($timeInThisSmil)}"
