    ...
    <xsl:template match="/*">
        ...
        <manifest xmlns="http://www.idpf.org/2007/opf">
            <xsl:for-each select="child::*">
                <item xmlns="http://www.idpf.org/2007/opf" href="{@href}" media-type="{@media-type}"
                    id="{concat('item_',position())}">
                    <xsl:if
                        ...
                        <xsl:attribute name="media-overlay"
                            select="concat('item_',count(($fileset/*[@href=$smil]/preceding-sibling::*))+1)"
                        />
                    </xsl:if>
                </item>
