 ...
 * category, associated to a priority so the matched rules can be ranked.
 */
public abstract class MatchRule {
     ...
     * 
     * @param category
     *            the classification applied to the words that match the rule
     * @param priority
     *            allows us to select a rule when several rules match the word.
     *            High value means it will be chosen over lower ones. When two
     *            priorities are equal, the one that returns the longest match
     *            is chosen.
     */
