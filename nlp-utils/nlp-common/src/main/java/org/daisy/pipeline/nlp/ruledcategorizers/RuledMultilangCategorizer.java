...
public class RuledMultilangCategorizer extends RuleBasedTextCategorizer {
	...
	public void init(MatchMode matchMode) throws IOException {
		...
		// ==== LATIN WORDS =====
		List<String> latin = Arrays.asList("per se", "vice versa", "magnum opus", "alter ego",
		        "status quo", "et cetera", "ex libris", "ad infinitum", "de facto",
		        "ipso facto", "mea culpa", "persona non grata", "in situ", "in vitro",
		        "a priori", "a posteriori", "ad nauseam", "habeas corpus", "ad hoc",
		        "curriculum vitae", "pro rata");
		WordListMatchRule wlmr = new WordListMatchRule(Category.COMMON,
		        DICTIONARY_MAX_PRIORITY, true, mMatchMode, false, Locale.ENGLISH);
		wlmr.init(latin);
		addRule(wlmr);
