...
public class PxiEvalCounterStep extends DefaultStep implements XProcStep {

	@Component(
		name = "pxi:eval-counter",
		service = { XProcStepProvider.class },
		property = { "type:String={http://www.daisy.org/ns/pipeline/xproc/internal}eval-counter" }
	)
	public static class Provider implements XProcStepProvider {
		
		@Override
		public XProcStep newStep(XProcRuntime runtime, XAtomicStep step, XProcMonitor monitor, Map<String,String> properties) {
			return new PxiEvalCounterStep(runtime, step);
		}
	}
	...
	private PxiEvalCounterStep(XProcRuntime runtime, XAtomicStep step) {
		super(runtime, step);
	}
			...
			Map<String,CounterStyle> namedCounterStyles = Maps.transformValues(
				...
				s -> {
					if (!(s instanceof Stylesheet))
						throw new IllegalArgumentException();
					if (!(((Stylesheet)s).style.underlyingObject instanceof CounterStyle))
						throw new IllegalArgumentException();
					return (CounterStyle)((Stylesheet)s).style.underlyingObject; });
				...
				if (attributeName.equals(element.getAttributeName(i))) {
					...
					if (d instanceof PropertyValue) {
						...
						if (pv.getValue() instanceof CounterSetList)
							return (List<CounterSet>)(List<?>)(CounterSetList)pv.getValue(); }
					break;
				}
			return null;
		}
