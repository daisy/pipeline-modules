...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	<xsl:template match="/metadata">
		<xsl:copy>
				...
				<meta property="dcterms:dateCopyrighted">
					<xsl:choose>
						...
						<!-- otherwise if dc:source is unknown and dc:date is present in the DTBook, take that value 
						     (ignore dc:source generated by dtbook-to-zedai) -->
						<xsl:when test="not($dtbook-metadata//dtb:meta[lower-case(@name)='dc:source']) and
			...
			<xsl:if test="not($dtbook-metadata//dtb:meta[lower-case(@name)='dc:source'])">
			...
			<xsl:if test="$dtbook-metadata//dtb:meta[lower-case(@name)='dc:source']">
				<xsl:variable name="id" select="(dc:source[1]/@id,'src')[1]"/> <!-- OPF's dc:source come from DTBook -->
	...
	<!-- Ignore dc:source that are generated by dtbook-to-zedai (from the DTBook identifier) -->
	<!-- Give the first dc:source an ID so that it can be referenced by dcterms:publisher -->
	<xsl:template match="dc:source">
		<xsl:if test="$dtbook-metadata//dtb:meta[lower-case(@name)='dc:source']">
			<xsl:copy>
				<xsl:if test="not(@id) and not(preceding-sibling::dc:source)">
