...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:f="http://www.daisy.org/ns/pipeline/internal-functions" xmlns:d="http://www.daisy.org/ns/pipeline/data" exclude-result-prefixes="#all" version="2.0">
        ...
        <xsl:element name="xsl:stylesheet">
            ...
            <xsl:for-each select="/*/d:element">
                ...
                <xsl:element name="xsl:template">
                    ...
                    <xsl:element name="xsl:copy-of">
                        <xsl:attribute name="select" select="concat(string-join(for $a in ($allowedAttributes) return concat('@',$a),'|'),(if (count($allowedAttributes) &gt; 0) then '|' else ''),&quot;@*[not(namespace-uri()=('','&quot;,$htmlns,&quot;')) or starts-with(local-name(),'data-')]&quot;)"
                        />
                    </xsl:element>
                    <xsl:element name="xsl:for-each">
                        ...
                        <xsl:element name="xsl:choose">
                            <xsl:element name="xsl:when">
                                <xsl:attribute name="test" select="concat(string-join(for $e in ($allowedElements) return concat('self::h:',$e),'|'),(if (count($allowedElements) &gt; 0) then '|' else ''),&quot;self::*[not(namespace-uri()=('','&quot;,$htmlns,&quot;'))]&quot;)"/>
                                <xsl:element name="xsl:copy">
                                    <xsl:element name="xsl:apply-templates">
                                        <xsl:attribute name="select" select="'.'"/>
                                    </xsl:element>
                            ...
                            <xsl:element name="xsl:when">
                                ...
                                <xsl:element name="xsl:choose">
                                    <xsl:element name="xsl:when">
                                        <xsl:attribute name="test" select="concat(&quot;namespace-uri()=('','&quot;,$htmlns,&quot;') and $replaceWith=('&quot;,string-join($allowedElements,&quot;','&quot;),&quot;')&quot;)"/>
