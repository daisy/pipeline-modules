...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:pf="http://www.daisy.org/ns/pipeline/functions" xmlns:d="http://www.daisy.org/ns/pipeline/data" exclude-result-prefixes="#all"
    version="2.0">
    ...
    <xsl:function name="pf:is-xml" as="xs:boolean">
        ...
        <xsl:value-of select="ends-with($media-type,'+xml') or ends-with($media-type,'/xml') or $media-type=('application/smil')"/>
    </xsl:function>
    
    <xsl:function name="pf:is-html" as="xs:boolean">
        <xsl:param name="media-type" as="xs:string"/>
        <xsl:value-of select="$media-type=('text/html','application/xhtml+xml')"/>
    </xsl:function>
    
    <xsl:function name="pf:is-text" as="xs:boolean">
        <xsl:param name="media-type" as="xs:string"/>
        <xsl:value-of select="not(pf:is-xml($media-type)) and starts-with($media-type,'text/')"/>
    </xsl:function>

    <xsl:function name="pf:is-binary" as="xs:boolean">
        <xsl:param name="media-type" as="xs:string"/>
        <xsl:value-of select="not(pf:is-xml($media-type) or pf:is-text($media-type) or pf:is-html($media-type))"/>
    </xsl:function>
