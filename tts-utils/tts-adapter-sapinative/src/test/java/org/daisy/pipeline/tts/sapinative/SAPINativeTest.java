...
public class SAPINativeTest {
    ...
    public static void load(){
		String libpath = System.getProperty("user.dir")+"/"+"target/generated-resources/lib/";
	...
	static String SSML(String x){
		return "<speak version=\"1.0\" xmlns=\"http://www.w3.org/2001/10/synthesis\"><s>"+x+"</s></speak>";
	}
	...
	public void twoBookmarks(){
		String b1 = "bmark1";
		String b2 = "bmark2";
		long connection = speakCycle(SSML("one two three four <mark name=\""+b1+"\"/> five six <mark name=\""+b2+"\"/> seven"));
		String[] names = SAPILib.getBookmarkNames(connection);
		long[] pos = SAPILib.getBookmarkPositions(connection);
		SAPILib.closeConnection(connection);
	...
	public void multithreadedSpeak() throws InterruptedException {
		final String[] sentences = new String[]{SSML("short"),
												SSML("regular size"),
												SSML("a bit longer size"),
